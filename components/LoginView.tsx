...
export default function LoginView() {
  ...
  const handleGoogleSignIn = async () => {
    setLoading(true);
    setError(null);
    const res = await googleLogin('alex.google@company.com', 'Alex Morgan', 'manager');
    if (!res.success) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between font-sans selection:bg-indigo-500 selection:text-white relative overflow-hidden">
      ...
      <main className="max-w-7xl w-full mx-auto px-4 sm:px-8 py-8 sm:py-12 flex-1 flex flex-col justify-center">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          {/* Right Column: Sleek Glassmorphic Sign In / Sign Up Form with Google SSO (Cols 5) */}
          <div className="lg:col-span-5">
            <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-7 shadow-2xl backdrop-blur-xl relative">
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-2.5 px-4 bg-slate-950 hover:bg-slate-800 border border-slate-700/80 rounded-xl text-xs font-bold text-slate-200 shadow-sm transition-all flex items-center justify-center space-x-2.5 mb-4 group"
              >
